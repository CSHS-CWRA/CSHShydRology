...
included.
In this case, all years are complete after 1926.
...
method can be employed instead by passing the argument `method = 'lmom'`.
...
However, in some occasions symmetrical confidence intervals can be unrealistic.  
In this case, confidence intervals provided by the profile likelihood method are
...
Without declustering, exceedances are likely to found in clusters 
associated with the same hydrological event.
Two function are available to extract independent peaks: `which.cluster` and 
`which.floodPeaks`. 
...
Such method may is more appropriate for rainfall events that are separated 
by sunny days. 
...
It is seen that the choice of a declustering method has a significant impact
...
In the fitted model above, the shape parameter is positive, which 
indicates the distribution has an upper bound $u + \alpha/\kappa$.
The flood quantiles $x_T$ of a $T$ year return period event is defined as the 
quantile of probability $[1-1/(\lambda T)]$ of the exceedance distribution, where $\lambda$ is the excess rate (average number of exceedances per year).
A natural estimator of $\lambda$ is the fraction of flood events over the number
of years of observations.
It was determinded that for a GPA distribution the flood quantiles are given by
$$
x_T = u + \frac{\alpha}{\kappa}\left[ 1 - \left(\lambda T\right)^{-\kappa}
\right].
$$
The flood quantiles can be evaluated using the function `predict`, where
...
confidence intervals of the flood quantiles.
...
that if $u$ is a well-chosen threshold then the 
...
stability. 
One visual assessment of the threshold stability is provided by the mean residual 
life plot (MRL).
...
For the common significance level of 0.05, it was found that such procedure 
...
threshold associated to a given excess rate may lead to samples of 
data that are not well approximated by a GPA.
...
The arguments `sgn-max` or `sgn-ppy` ask to `FindThresh` to search for the 
`sgn` threshold and if it cannot be found, will return the alternative 
candidates.
...
This serve as an additional verification that the flood quantile tend to remain
...
bound of 1 PPY which lead to a reference flood quantile around 1 PPY.
...
and reference treshold is less than 1%.
...
In this document, it was shown how a threshold modeling can be carried out  
to evaluate flood quantiles.
Instructions for fitting the model (`FitPot`), evaluate uncertainty of 
