...
For this analysis, pooling groups are formed as nearest sites of a target
from which regional information will be transfered. 
...
may be considered.
The example below shows how to evaluate these distances.
Notice that the descriptors are transformed and scaled before the
Euclidean distance is computed. 
The `Geodist` function compute a great-circle distance between point.
...
Another measure of similarity is based on the seasonality of the annual 
maximums that characterizes the flood regime the site of interest.
...
\end{align}
$$
Here, $\theta$ represents the average moment of occurrence for the annual
...
One region includes sites with nival regimes (blue), _i.e._ annual floods 
are occurring more regularly around May.
A second region (green) represents sites with pluvial regime, _i.e._ with a 
lower regularity and where rainfall events are occurring during winter.
...
Better inference for the estimated flood quantiles can be obtained by 
...
scale using the at-site L-moments.
...
Due to the spatial nature of flood generating process 
...
However to obtain reliable confidence intervals, a bootstrap method is necessary
The argument `corr` can be the correlation matrix of a multivariate Normal 
distribution used for simulation. 
It can also be a single value, in which case a constant coefficient of 
correlation is used.
