...
For this purpose, flood quantiles need to be predicted according to 
...
in order to obtain at-site estimates of the flood quantiles.
...
distribution based on at-site estimates of the same parameters.
...
the choice must depend on practical considerations. (Ahn and Palmer, 2016; 
Haddad and Rahman, 2012).

Here the QRT approach is adopted to predict a 100 years return period using a
...
site coordinates in a 2D space that tends preserves the great-circle distance.
...
The dataset does not contain the annual maximums of each sites but provide the
sample L-moments. 
...
Note below that the presence of the argument `scale = FALSE` that specified that the LCV is passed rather than the L-scale.
...
For the rest of the document, we put aside 26 sites that will be considered 
as ungauged and used the other sites to predict them.
...
The latter could be catchment descriptors, coordinates or a mixed of both.
...
\end{cases},
$$
which are proportional to an Epechnikov kernel where the bandwidth is 
the $n_k$-th nearest site.
This give more importance to the sites having more similar attributes.
This create neighborhoods, or regions of influence (ROI) where only 
...
of the catchment descriptors and $\epsilon$ is a term of error.
...
The exemple below shows how to fit the local regression models on the 
ungauged sites.
...
The k-fold cross-validation strategy consists to divide the gauged sites in k 
...
common criteria such as the Root Mean Square Error (RMSE) and the Mean Absolute 
Deviation (MAD).
The function `CvRoi` performs cross-validation and return the evaluation 
criteria. The function `head` return the three best choice for `nk` according 
to a given criteria.
...
If other criteria than those included in `CvRoi` are desired, it is 
possible to perform the cross-validation sheme and keep the predicted value (or 
residuals) using functions `predict` (`residuals`).
...
Considering $\mathbf{z} = \mathbf{y}-g(\mathbf{X})$, 
the residuals of the local regression. 
The simple kriging estimator, is the linear predictor $z_0 = \mathbf{a}'\mathbf{z}$, that minimize the predicted variance and is given by
$$
\mathbf{a} = \Sigma^{-1}\sigma
$$
where $\sigma$ is the covariance matrix of the residual and 
$\sigma = \mathrm{cov}(\mathbf{z},z_0)$.
This requires the estimation of a covariance model, which is treated in 
textbooks such as Schabenberger and Gotway (2014).
One model of covariance with respect to the distance $h > 0$ is the exponential 
model 
$$
C(h) = C(0) -\tau \exp\left(-3 \frac{h}{\alpha}\right)
$$
where $\alpha > 0$ is a parameter that characterizes 
...
standard deviation for the regression part (`phy.se`) or the kriging part 
(`krige.se`) of the model. 
These outputs may be useful to assess the quality of the fitting, but they 
don't give a proper evaluation of the total uncertainty.
Indeed, errors must be propagate at each step.
Overall the model could be seen as having two terms of errors
$$
Y = f(\mathbf{s}) + \eta + \omega
$$
where $\eta$ is a term of sampling error and $\omega$ is a term of modeling 
error.
A similar hypothesis is used in this context when
using Generalized Least Squares (GLS) for estimating of the total variance 
(Tasker and Stedinger, 1989; Durocher et al., 2018).
...
independently each terms of error. 

The example below simulates the gauged sites using the function `RegSim`.
Here, 50 years of data is employed with
a constant coefficient of corrrelation of 0.4 representing the intersite 
correlation between the annual maximums.
At each time, the at-site flood quantiles are evaluated using the 
method of L-moments.
Here only a small sample of size `nboot` is generated for illustration purpose.
...
A bootstrap sample of the total error is obtained then 
by adding the model residuals to the sample from the at-site analyses. 
Note that if kriging is not required, the argument `fitted = TRUE` must be 
...
The selection of the catchment descriptors and the size of the region of 
