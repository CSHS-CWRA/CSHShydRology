...
#' 
#' @details
#' The function has two gridded inputs: a DEM (with sinks removed as appropriate) and a grid of contributing areas. Other inputs include \code{outlet}, a sf object containing the coordinates of the catchment outlets, and \code{outlet_label}, an optional vector of character strings that serve as labels or descriptions of the outlet points.
#' 
#' The DEM is input as a raster object via the \code{dem} argument.
#' 
#' There are three ways to provide the contributing area grid, which can be accomplished using the \code{carea} and \code{carea_flag} arguments, as summarized below. 
#' 
#'   1. The first option (\code{carea = NULL} and \code{carea_flag = 0}) would be appropriate if you have not generated a contributing area grid prior to application of \code{ch_saga_catchment}. If you use this option, the contributing area grid is generated using the standard SAGA defaults, with no options for the user to change them.  
#' 
#'   2. The second option (\code{carea = NULL} and \code{carea_flag = 1}) would be appropriate if you have run the \code{ch_saga_carea} function and have not cleared the working directory.  
#' 
#'   3. The third option would be appropriate if you have generated a contributing area grid as a raster object using either \code{ch_saga_carea} or another function.  
#' 
...
#' @param outlet_label character vector of labels; if \code{NULL}, numbers are assigned
...
ch_saga_catchment <- function(dem, saga_wd, outlet,
  ...
  if (missing(saga_wd)) {
    saga_wd <- tempdir()
    warning(sprintf("ch_saga_catchment: no saga_wd defined; setting temporary saga_wd with tempdir as:\n%s",saga_wd))
  }
