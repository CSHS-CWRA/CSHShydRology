...
#'
#' @references Gustard, A., A. Bullock, and J.M. Dixon. 1992. Low flow estimation in the United Kingdom. Institute of Hydrology, 292. Wallingford: Institute of Hydrology.
#' @references Vogel, R.M., and N.M. Fennessy. 1994. Flow-duration curves. I: New Interpretation and confidence intervals. Journal of Water Resources Planning and Management ASCE 120:485-504.
#' @references Vogel, R.M., and N.M. Fennessy. 1995. Flow duraction curves II: A review of applications in water resources planning. Water Resources Bulletin 31:1030-9.
#'
#'
#' @export 
#' @examples
#' 
#' \dontrun{test <-fdcurve(flow, title, normal=FALSE, gust=TRUE)}
#' 
#
...
fdcurve <- function(flow, title, normal=FALSE, gust=TRUE) {
  ...
  if(normal==TRUE) {
    ...
    xlims <-c(-3,3)
    graphics::plot(exceed.z, q, type='l', lwd=2, col='blue', log="y", xaxt="n",ylim=ylims,xlim=xlims,xlab=xl,
