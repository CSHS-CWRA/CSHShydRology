...
#' @param channels channel polyline (or channels list from \code{ch_wbt_channels}) (\pkg{terra} \code{SpatVector} object)
...
ch_checkchannels <- function(dem, channels, outlet = NULL, main_label = "",
  ...
  if (missing(channels)) {
    stop("ch_checkchannels requires SpatVector channels polyline to plot")
  }
  if (missing(outlet)) {
    stop("ch_checkchannels requires SpatVector outlet to plot")
  }
