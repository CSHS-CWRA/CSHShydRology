...
#'@param input.hrs PMP to be defined 24 hrs, 6 hrs or 1 hr.
...
#' }
#' Please note that data with the value \code{NA} are removed in the analysis.
#'@examples
#' ## EX 1: PMP for the daily maximum values for a 24 hours PMP in an area of 200km2 
#' ## (Optional for Western US).
#' ## Random 30 number from 60 to 150
#' ch_pmp_hershfield(val.max = runif(30,min = 60,150),input.hrs = 24,number.of.obs.unit = 1,area.km = 200,graphs = T)
...
#'@author Victor Munoz
#'@export
#'@importFrom ggplot2 geom_line aes scale_x_continuous scale_y_continuous scale_color_discrete labs geom_point geom_text
ch_pmp_hershfield  <-  function(val.max, input.hrs=24, number.of.obs.unit=1, area.km=1, graphs=T){
  ...
  if(graphs==T){
    ...
    Fg4.2 <- ggplot(data=Fig_4.2.df,aes(x=Xn_m,y=Xn_adj))+
      ...
      labs(x="Xn-m/Xn",y="Xn adjustment factor",color="Length of\nrecord\n[yrs]",
    ...
    Fg4.3 <- ggplot(data=Fig_4.3.df,aes(x=Sn_m,y=Sn_adj))+
      ...
      scale_color_discrete()+
      labs(x="Sn-m/Sn",y="Sn adjustment factor",color="Length of\nrecord\n[yrs]",
    ...
    Fg4.4 <- ggplot(data=Fig_4.4.df,aes(x=n_graph,y=Xn_Sn))+
      ...
      scale_color_discrete()+
      labs(x="Length of record [years]",y="Adjustment factor",color="Param.",
              title="Figure 4.4 - WMO No 1045 (2009)",
              subtitle="Adjust. of mean and SD of annual series for length of record")+
      geom_point(data=Fig_4.4.site,aes(x=n_PMP,y=Xn_Sn),size=3,color="red")
    ...
    Fg4.1 <- ggplot(data=Fig_4.1.df,aes(x=mamr,y=Km))+
      ...
      scale_color_discrete()+
      labs(x="Mean Annual Maximum Rainfall [mm]",y="Km",color="Storm\nDuration\n[hrs]",
    ...
    Fg4.7 <- ggplot(data=Fig_4.7.df,aes(x=area,y=ppm))+
      ...
      labs(x="Area[km2]",y="Percentage of probable maximum\npoint, or 24 km2, rainfall",
