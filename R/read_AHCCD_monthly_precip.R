#' Reads AHCCD monthly precipitation file
#'
#' @description This program reads Adjusted and Homogenized Canadian Climate Data (AHCCD) data of 
#' rainfall, snowfall or total precipitation. The values are arranged as year x month, 
...
#' the precipitation data must cite \cite{Mekis, E and L.A. Vincent, 2011: An overview of 
#' the second generation adjusted daily precipitation dataset for trend analysis in Canada. 
#' Atmosphere-Ocean, 49 (2), 163-177.}
#' @examples
#' \dontrun{
#' stoon_monthly_total <- read_AHCCD_monthly_precip("mt4057120.txt")}
#' @export

read_AHCCD_monthly_precip <- function(precip_file) {
  # check parameter
  ...
  frenchLineNum <- stringr::str_detect(input,stringr::fixed("annuel", ignore_case=TRUE))
  ...
  # read data
  raw <- utils::read.csv(file = precip_file, header = FALSE, skip = totalSkipLines)
  row_count <- nrow(raw)
  # now unstack data
  data_row <- 0
  ...
  names(good_data) <- c("year", "month", precip_type, "code")
  return(good_data)
}
