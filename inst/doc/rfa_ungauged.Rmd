...
Afterward, the flood quantiles of the target site is derived from the
...
Note that the prediction of each parameter (or L-moments) would follow the 
same steps.
...
Note below that the presence of the argument `scale = FALSE` that specifies 
that the LCV is passed rather than the L-scale.
...
This create a neighborhood, or regions of influence (ROI) where only 
...
of the descriptors and $\epsilon$ is a term of error.
...
Deviation (MAD).
The function `CvRoi` performs cross-validation and return the evaluation several evaluation criteria. 
...
A regression model provide a useful way to describe the relation
...
Considering $\mathbf{z} = \mathbf{y}-g(\mathbf{X})$, the residuals of the local regression, the simple kriging estimator, is the linear predictor $z_0 = \mathbf{a}'\mathbf{z}$ that minimize the predicted variance and is given by
$$
...
the rate at which the covariance falls with respect to $h$ and 
...
The argument `se = TRUE` can be passed to `FitRoi` to return the 
...
Indeed, errors must be propagate at each step of the procedure.
...
estimating of the total variance (Tasker and Stedinger, 1989; 
Durocher et al., 2018).
