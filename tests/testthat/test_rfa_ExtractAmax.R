...
a <- ExtractAmax(xd0, tol = 360)

expect_equal(a$yy, as.character(c(1971:1999,2001:2010)))

## verify formula work
a <- ExtractAmax(value ~ date, xd0, tol = 360, nlab = NULL, ylab = NULL)
expect_equal(colnames(a), c('value','date'))

## Multiple sites
xd1 <- rbind(cbind(xd0, site = 1), cbind(xd0, site = 2))

a0 <- ExtractAmax(value ~ site + date, xd1, tol = 360, nlab = NULL, ylab = NULL)
...
a0 <- ExtractAmax(value ~ site + date, xd1, nlab = 'nn', ylab = NULL)
